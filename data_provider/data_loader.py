...
class Dataset_5G_minute(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 features='M', data_path='ETTm1.csv',
                 target='Bitrate', scale=True, timeenc=0, freq='t'):
    ...
    def __read_data__(self):
        ...
        df_raw = pd.read_csv(os.path.join(self.root_path,
                                          self.data_path))
        length = len(df_raw)
        border1s = [0, int(length*0.8)- self.seq_len, int(length*0.9) - self.seq_len]
        border2s = [int(length*0.8), int(length*0.9), length]
        ...
        # Ensure 'datetime' column is excluded before scaling
        if self.scale:
            train_data = df_data[border1s[0]:border2s[0]]  # Exclude `datetime`
        ...
        df_stamp['datetime'] = pd.to_datetime(df_stamp['datetime'])  # Correct column name
        ...
        if self.timeenc == 0:
            ...
            df_stamp['minute'] = df_stamp['datetime'].apply(lambda row: row.minute)
            data_stamp = df_stamp.drop(['datetime'], axis=1).values  # Drop `datetime`
...
class Dataset_Pred(Dataset):
    ...
    def __read_data__(self):
        ...
        df_stamp.datetime = list(tmp_stamp.datetime.values) + list(pred_dates[1:])
        if self.timeenc == 0:
            ...
            df_stamp['minute'] = df_stamp.minute.map(lambda x: x)
